...
/**
 * Another smarter kind of bot, which implements a minimax algorithm with
 * look-ahead of two turns. It simulates the opponent using the BullyBot
 * strategy and simulates the possible outcomes for any choice of source and
 * destination planets in the attack. The simulated outcome states are ranked by
 * the evaluation function, which returns the most promising one.
 * 
 * Try to improve this bot. For example, you can try to answer some of this
 * questions. Can you come up with smarter heuristics/scores for the evaluation
 * function? What happens if you run this bot against your bot from week1? How
 * can you change this bot to beat your week1 bot? Can you extend the bot to
 * look ahead more than two turns? How many turns do you want to look ahead? Is
 * there a smart way to make this more efficient?
 */
...
public class LookaheadBot {
	...
	public static void DoTurn(PlanetWars pw) {
		...
		// We try to simulate each possible action and its outcome after two
		// turns
		...
		for (Planet myPlanet : pw.MyPlanets()) {
			...
			for (Planet notMyPlanet : pw.NotMyPlanets()) {
				...
				// Create simulation environment - need to create one for each
				// simulation
				SimulatedPlanetWars simpw = createSimulation(pw);

				// (1) simulate my turn with the current couple of source and
				// destination
				simpw.simulateAttack(myPlanet, notMyPlanet);
				// (2) simulate the growth of ships that happens in each turn
				simpw.simulateGrowth();

				// (3) simulate the opponent's turn, assuming that the opponent
				// is the BullyBot
		...
		// Attack using the source and destinations that lead to the most
		// promising state in the simulation
	...
	/**
	 * This function evaluates how promising a simulated state is. You can
	...
	/**
	 * Static LookaheadBot, used only to access SimulatedPlanetWars (DON'T
	 * CHANGE)
	 */
	static LookaheadBot dummyBot = new LookaheadBot();

	/**
	 * Class which provide the simulation environment, has same interface as
	 * PlanetWars (except for Fleets, that are not used).
	 *
	 */
	public class SimulatedPlanetWars {
		...
		public void simulateBullyBotAttack() {
			...
			for (Planet planet : planets) {
				if (planet.Owner() == 2) {// skip planets with only one ship
					...
					// This score is one way of defining how 'good' my planet
					// is.
					double scoreMax = (double) planet.NumShips();

					if (scoreMax > sourceScore) {
						// we want to maximize the score, so store the planet
						// with the best score
				...
				if (planet.Owner() != 2) {
					...
					// if you want to debug how the score is computed, decomment
					// the System.err.instructions
					// System.err.println("Planet: " +notMyPlanet.PlanetID()+
					// " Score: "+ score);
					// System.err.flush();
					if (scoreMin < destScore) {
						// The way the score is defined, is that the weaker a
						// planet is, the higher the score.
						// So again, we want to select the planet with the best
						// score
						destScore = scoreMin;
						dest = planet;
					}
		...
		// Return a list of all the planets owned by rival players. This
		// excludes
		...
		// Returns the distance between two planets, rounded up to the next
		// highest
		...
		// fleets remaining), returns -1. If the game is over (ie: only one
		// player
		...
		// Returns the number of ships that the current player has, either
		// located
